...
export const getBanners = async (req, res) => {
    ...
    try {
        ...
        if (banners.length === 0) return res.status(404).json({ Message: "We have no banners at this moment" });
...
export const getBannerByTitle = async (req, res) => {
    ...
    if (!title) return res.status(400).json({ message: "The banner's title is required to search for" });
    ...
    if (reqBanner.length === 0) return res.status(404).json({ message: "This banner doesn't exist" });

    res.status(200).json({ message: "Required banner fetched successfully", reqBanner })
    }catch(error){
        res.status(500).json({ error: error.message });
    }
