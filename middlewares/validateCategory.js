...
export const validateCategoryId = async (req, res, next) => {
    ...
    if (!foundedCategoryById) return res.status(404).json({ Message: "Category does not exist." });
...
export const validateCategoryName = async (req, res, next) => {
    ...
    if (foundedCategoryByName) return res.status(409).json({ Message: "category already exists." });
